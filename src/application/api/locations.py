...
@locations_router.get(
    "/", status_code=status.HTTP_200_OK, response_model=List[Location]
)
...
@locations_router.get(
    "/id/{location_id}", status_code=status.HTTP_200_OK, response_model=Location
)
...
@locations_router.get(
    "/name/{name}", status_code=status.HTTP_200_OK, response_model=Location
)
async def get_location_by_name(
    name: str, use_case=Depends(get_location_by_name_use_case)
) -> Location:
    try:
...
async def create_location(
    location_data: CreateLocation, use_case=Depends(create_location_use_case)
) -> Location:
    try:
