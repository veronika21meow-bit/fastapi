...
@categories_router.get(
    "/", status_code=status.HTTP_200_OK, response_model=List[Category]
)
...
@categories_router.get(
    "/id/{category_id}", status_code=status.HTTP_200_OK, response_model=Category
)
async def get_category_by_id(
    category_id: int, use_case=Depends(get_category_by_id_use_case)
) -> Category:
    try:
...
@categories_router.get(
    "/slug/{slug}", status_code=status.HTTP_200_OK, response_model=Category
)
async def get_category_by_slug(
    slug: str, use_case=Depends(get_category_by_slug_use_case)
) -> Category:
    try:
...
@categories_router.get(
    "/title/{title}", status_code=status.HTTP_200_OK, response_model=Category
)
async def get_category_by_title(
    title: str, use_case=Depends(get_category_by_title_use_case)
) -> Category:
    try:
...
async def create_category(
    category_data: CreateCategory, use_case=Depends(create_category_use_case)
) -> Category:
    try:
    ...
    except CategoryTitleIsNotUniqueException as exc:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail=exc.get_detail()
        )
