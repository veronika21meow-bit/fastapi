...
@comments_router.get(
    "/{comment_id}", status_code=status.HTTP_200_OK, response_model=Comment
)
...
@comments_router.get(
    "/post/{post_id}", status_code=status.HTTP_200_OK, response_model=List[Comment]
)
async def get_comments_by_post(
    post_id: int, use_case=Depends(get_comments_by_post_use_case)
) -> List[Comment]:
    try:
        comment = await use_case.execute(post_id=post_id)
        return comment
...
async def create_comment(
    comment_data: CreateComment, use_case=Depends(create_comment_use_case)
) -> Comment:
    try:
